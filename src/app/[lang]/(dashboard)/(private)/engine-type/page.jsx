...
const EngineType = () => {
  ...
  const handleSaveCategory = async (categoryData, id) => {
    try {
      ...
      await fetchEngine() // Refresh data in the table
    } catch (error) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          console.error('Delete engine type error:', error)

          const errorMsg = error.response?.data?.message || 'Failed to delete engine type.'

          toast.error(errorMsg)
        }
      } else if (result.dismiss === Swal.DismissReason.cancel) {
        toast.info('engine type deletion cancelled.')
      }
    })
  }
      ...
      <Card sx={{ p: '1.5rem' }}>
        <div style={{ marginBottom: 10, paddingBottom: 10, borderBottom: `1px solid ${theme.palette.divider}` }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
            <span style={{ fontSize: 16, fontWeight: 500, color: theme.palette.text.primary }}>Engine Type</span>
            <Button
          ...
          >
            ...
            {' / '}
            <Link href='/engine-type' style={{ textDecoration: 'none', color: theme.palette.text.primary }}>
              Engine Type
            </Link>
          </div>
        </div>
