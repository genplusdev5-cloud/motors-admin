...
import { getYear, addYear, updateYear, deleteYear } from '@/services/recordeYearApi'
...
const RecordYear = () => {
  ...
  const fetchYear = useCallback(async () => {
    ...
    try {
      const categoryData = await getYear()
  ...
  const handleSaveCategory = async (categoryData, id) => {
    try {
      ...
      const isDuplicate = data.some(
        item => item.name?.trim().toLowerCase() === categoryData.name?.trim().toLowerCase() && item.id !== id // allow same record during update
      )
      ...
      if (id) {
        await updateYear(id, categoryData)
        toast.success('Year updated successfully!')
      } else {
        await addYear(categoryData)
      ...
      // ✅ Handle backend duplicate or warning message
      let errorMsg = error.response?.data?.message || 'An error occurred while saving the Year.'

      if (errorMsg.toLowerCase().includes('already exist') || errorMsg.toLowerCase().includes('duplicate')) {
        errorMsg = 'Year name already exists.'
      ...
      <Card sx={{ p: '1.5rem' }}>
        <div style={{ marginBottom: 10, paddingBottom: 10, borderBottom: `1px solid ${theme.palette.divider}` }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
            <span style={{ fontSize: 16, fontWeight: 500, color: theme.palette.text.primary }}>Record Year</span>
            <Button
          ...
          >
            ...
            {' / '}
            <Link href='/record-year' style={{ textDecoration: 'none', color: theme.palette.text.primary }}>
              Record Year
            </Link>
          </div>
        </div>
