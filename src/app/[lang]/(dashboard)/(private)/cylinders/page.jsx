...
const Cylinder = () => {
  ...
  const fetchCylinder = useCallback(async () => {
    ...
    try {
      ...
      toast.error('Failed to load cylinder.')
      setData([])
    } finally {
      setLoading(false)
    }
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          const errorMsg = error.response?.data?.message || 'Failed to delete cylinder.'

          toast.error(errorMsg)
        }
      } else if (result.dismiss === Swal.DismissReason.cancel) {
        toast.info('cylinder deletion cancelled.')
      }
    })
  }
      ...
      <Card sx={{ p: '1.5rem' }}>
        <div style={{ marginBottom: 10, paddingBottom: 10, borderBottom: `1px solid ${theme.palette.divider}` }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
            <span style={{ fontSize: 16, fontWeight: 500, color: theme.palette.text.primary }}>Cylinder</span>
            <Button
