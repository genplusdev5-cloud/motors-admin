...
import { getCylinderList, addCylinder, updateCylinder, deleteCylinder } from '@/api/cylinder'
...
export default function CylinderPage() {
  ...
  const loadCylinders = async () => {
    setLoading(true)
    try {
      const res = await getCylinderList()
      const list = res?.data || []
      ...
      showToast('error', err.response?.data?.message || 'Failed to load cylinders')
      setRows([])
    } finally {
      setLoading(false)
    }
  ...
  const handleSubmit = async e => {
    e.preventDefault()
    if (!formData.name.trim()) return showToast('warning', 'Cylinder name is required')

    if (!isEdit) {
      const exists = rows.some(r => r.name.trim().toLowerCase() === formData.name.trim().toLowerCase())
      if (exists) return showToast('warning', 'Cylinder already exists')
    }
    ...
    try {
      ...
      if (isEdit) {
        await updateCylinder(formData.id, payload)
        showToast('success', 'Cylinder updated successfully')
      } else {
        await addCylinder(payload)
        showToast('success', 'Cylinder added successfully')
      }
      ...
      showToast('error', err.response?.data?.message || 'Failed to save cylinder')
    } finally {
      setLoading(false)
    }
  }
  ...
  const confirmDelete = async () => {
    ...
    try {
      await deleteCylinder(deleteDialog.row.id)
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor('name', { header: 'Cylinder' }),
  ...
  const exportPrint = () => {
    ...
    const html = `
      <html><head><title>Cylinder List</title><style>
      ...
      <h2>Cylinder List</h2>
      <table><thead><tr><th>S.No</th><th>Cylinder</th><th>Description</th><th>Status</th></tr></thead><tbody>
  ...
  const exportCSV = () => {
    const headers = ['S.No', 'Cylinder', 'Description', 'Status']
    const csv = [
      headers.join(','),
    ...
    link.download = 'Cylinder_List.csv'
    link.click()
  ...
  const exportExcel = async () => {
    ...
    utils.book_append_sheet(wb, ws, 'Cylinders')
    writeFile(wb, 'Cylinder_List.xlsx')
  ...
  const exportPDF = async () => {
    ...
    const doc = new jsPDF()
    doc.text('Cylinder List', 14, 15)
    autoTable(doc, {
      startY: 25,
      head: [['S.No', 'Cylinder', 'Description', 'Status']],
      body: rows.map(r => [r.sno, r.name, r.description || '-', r.is_active == 1 ? 'Active' : 'Inactive'])
    })
    doc.save('Cylinder_List.pdf')
    ...
    <Box>
      ...
      <Box sx={{ mb: 2 }}>
        <Breadcrumbs aria-label='breadcrumb'>
          ...
          <Typography color='text.primary'>Cylinder</Typography>
        </Breadcrumbs>
      </Box>
        ...
        <Box sx={{ p: 5, display: 'flex', flexDirection: 'column', height: '100%' }}>
          <Box display='flex' justifyContent='space-between' alignItems='center' mb={3}>
            <Typography variant='h5' fontWeight={600}>
              {isEdit ? 'Edit Cylinder' : 'Add Cylinder'}
            </Typography>
      ...
      >
        ...
        <DialogContent sx={{ px: 5, pt: 1 }}>
          <Typography sx={{ color: 'text.secondary', fontSize: 14, lineHeight: 1.6 }}>
            ...
            <strong style={{ color: '#d32f2f' }}>{deleteDialog.row?.name || 'this cylinder'}</strong>?
            <br />
