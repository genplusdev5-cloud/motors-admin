...
import { toast } from 'react-toastify' // Ensure toast is imported if you use it for validation
...
const AddModelWindow = ({ open, setOpen, onSaveCategory, editingRow }) => {
  ...
  // State tracks all form fields. is_active defaults to '1' (Active) for new items.
  ...
  useEffect(() => {
    if (editingRow) {
      // Convert incoming is_active (1 or true) to the string '1' or '0' for the Select input
  ...
  const handleSave = () => {
    ...
    const formattedData = {
      ...
      // CRITICAL: Convert the string value ('1' or '0') from the dropdown back to a number (1 or 0)
      is_active: Number(data.is_active)
    }

    const id = isEdit && editingRow ? editingRow.id : null

    // Pass data and ID to the main component's API handler
    onSaveCategory(formattedData, id)

    // The modal is closed by the parent component (Category.js) only on success.
  }
  ...
  return (
    ...
    >
      {/* Assuming DialogCloseButton is a component that renders an IconButton with a close icon */}
      <DialogCloseButton onClick={handleClose}>
        <i className='tabler-x' />
      </DialogCloseButton>

      <DialogTitle sx={{ fontWeight: 600, pb: 1 }}>{isEdit ? 'Edit Category' : 'Add Category'}</DialogTitle>
