...
import { getMileageList, addMileage, updateMileage, deleteMileage } from '@/api/mileage'
...
export default function MileagePage() {
  ...
  const loadMileage = async () => {
    ...
    try {
      ...
      // Yeh line sabse important - response structure ke hisaab se
      ...
      setRows(normalized)
      console.log('Mileage loaded:', normalized) // Debug ke liye
    } catch (err) {
      console.error('Error loading mileage:', err)
      showToast('error', 'Failed to load mileage data')
      setRows([])
    } finally {
      setLoading(false)
    }
  ...
  const handleSubmit = async e => {
    e.preventDefault()
    if (!formData.name.trim()) return showToast('warning', 'Mileage name is required')

    if (!isEdit) {
      const exists = rows.some(r => r.name.trim().toLowerCase() === formData.name.trim().toLowerCase())
      if (exists) return showToast('warning', 'Mileage already exists')
    }
    ...
    try {
      ...
      if (isEdit) {
        await updateMileage(formData.id, payload)
        showToast('success', 'Mileage updated')
      } else {
        await addMileage(payload)
        showToast('success', 'Mileage added')
      }
      ...
      showToast('error', err.response?.data?.message || 'Failed to save mileage')
    } finally {
      setLoading(false)
    }
  }
  ...
  const confirmDelete = async () => {
    ...
    try {
      await deleteMileage(deleteDialog.row.id)
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor('name', { header: 'Mileage' }),
  ...
  // Export Functions
  const exportPrint = () => {
    /* same as before */ window.print()
  }
  const exportCSV = () => {
    const csv = [
      ['S.No', 'Mileage', 'Description', 'Status'],
      ...rows.map(r => [r.sno, r.name, r.description || '-', r.is_active == 1 ? 'Active' : 'Inactive'])
    ]
      .map(e => e.join(','))
      .join('\n')
    const link = document.createElement('a')
    link.href = 'data:text/csv;charset=utf-8,' + encodeURIComponent(csv)
    link.download = 'Mileage_List.csv'
    link.click()
  ...
  const exportExcel = async () => {
    ...
    utils.book_append_sheet(wb, ws, 'Mileage')
    writeFile(wb, 'Mileage_List.xlsx')
  ...
  const exportPDF = async () => {
    ...
    const doc = new jsPDF()
    doc.text('Mileage List', 14, 15)
    autoTable(doc, {
      startY: 25,
      head: [['S.No', 'Mileage', 'Description', 'Status']],
      body: rows.map(r => [r.sno, r.name, r.description || '-', r.is_active == 1 ? 'Active' : 'Inactive'])
    })
    doc.save('Mileage_List.pdf')
  ...
  return (
    <Box>
      <Box sx={{ mb: 2 }}>
        <Breadcrumbs>
          <Link href='/'>Home</Link>
          <Typography color='text.primary'>Mileage</Typography>
        </Breadcrumbs>
      </Box>
