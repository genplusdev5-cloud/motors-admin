...
export default function TyreSizePage() {
  const ENTITY = 'Tyre Size'
  ...
  const handleAdd = () => {
    setIsEdit(false)
    setFormData({ id: null, name: '', description: '', wheel_size: '', status: 1 })
    setDrawerOpen(true)
  ...
  const handleSubmit = e => {
    ...
    if (!formData.wheel_size.trim()) return showToast('warning', `Wheel Size required`)
          ...
          prev.map(r =>
            r.id === formData.id
              ? {
                  ...
                  wheel_size: formData.wheel_size.trim(),
  ...
  const confirmDelete = () => {
    ...
    try {
      setRows(prev => {
        const filtered = prev.filter(r => r.id !== deleteDialog.row.id)
        return filtered.map((r, index) => ({ ...r, sno: index + 1 }))
      })
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor('wheel_size', { header: 'Wheel Size' }),
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter(
      r =>
        ...
        r.wheel_size?.toLowerCase().includes(searchText.toLowerCase())
    )
  }, [rows, searchText])
          ...
          <form onSubmit={handleSubmit}>
            <Grid container spacing={3}>
              ...
              <Grid item xs={12}>
                <GlobalTextField
                  label='Wheel Size *'
                  placeholder='Ex: 17 inch'
                  value={formData.wheel_size}
                  required
                  onChange={e => handleFieldChange('wheel_size', e.target.value)}
                />
              </Grid>
