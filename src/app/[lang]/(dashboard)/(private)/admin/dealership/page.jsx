...
export default function DealerShip() {
  ...
  const handleSubmit = e => {
    ...
    if (isEdit) {
      setRows(prev =>
        prev.map(r =>
          r.id === formData.id
            ? { ...r, ...formData, status: formData.status ? 'Active' : 'Inactive' }
            : r
        )
      )
    } else {
  ...
  const columns = [
    ...
    columnHelper.accessor('name', { header: 'Dealership Name' }),
  ...
  return (
    <Box>
      ...
      <Breadcrumbs sx={{ mb: 2 }}>
        <Link href='/'>Home</Link>
        <Typography color='text.primary'>Dealership</Typography>
      </Breadcrumbs>

      <Card sx={{ p: 3 }}>
        <CardHeader
          title={<Typography variant='h5' fontWeight={600}>Dealership</Typography>}
          action={
            <Box display='flex' gap={2}>
              <GlobalButton startIcon={<RefreshIcon />} loading={loading} onClick={loadData}>
                Refresh
              </GlobalButton>
