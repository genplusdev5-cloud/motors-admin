...
import { getVehicle, addVehicle, updateVehicle, deleteVehicle } from '@/services/vehicleTypeApi'
...
const VehicleType = () => {
  ...
  const fetchVehicleType = useCallback(async () => {
    ...
    try {
      const categoryData = await getVehicle()
  ...
  const handleSaveCategory = async (categoryData, id) => {
    try {
      ...
      await fetchVehicleType() // Refresh data in the table
    } catch (error) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          const errorMsg = error.response?.data?.message || 'Failed to delete vehicle type.'
      ...
      <Card sx={{ p: '1.5rem' }}>
        <div style={{ marginBottom: 10, paddingBottom: 10, borderBottom: `1px solid ${theme.palette.divider}` }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
            <span style={{ fontSize: 16, fontWeight: 500, color: theme.palette.text.primary }}>Vehicle Type</span>
            <Button
